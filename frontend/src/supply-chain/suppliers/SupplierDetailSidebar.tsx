...
function LinkImprintWidget({
  ...
  useEffect(() => {
    ...
    fetchSuppliers({ search: query, activeOnly: false })
      .then(r => setResults(
        r.filter(p => p.id !== parentId && !existingChildIds.has(p.id)).slice(0, 8)
      ))
      .catch(() => {})
            ...
            <div className="absolute z-10 left-0 right-0 bg-white dark:bg-gray-900 border dark:border-gray-700 rounded shadow-xl overflow-hidden">
              {results.map(p => (
                ...
                >
                  ...
                  {p.parent_id && (
                    <span className="text-gray-400 ml-1">(already has parent)</span>
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-3">
              ...
              {party.website && (
                <div className="flex flex-col py-1 border-b border-gray-50 dark:border-gray-800">
                  ...
                  <a
                    ...
                    className="text-blue-500 hover:underline mt-0.5 text-sm truncate"
                  >
                    {party.website}
                  </a>
                </div>
              ...
              {party.shopify_vendor_codes && party.shopify_vendor_codes.length > 0 && (
                <div className="flex flex-col py-1">
                  <span className="text-[10px] uppercase tracking-wider text-gray-400 dark:text-gray-500 font-bold mb-1.5">
                    Vendor codes
                  </span>
                  <div className="flex flex-wrap gap-1">
                    {party.shopify_vendor_codes.map(code => (
                      <span
                        key={code}
                        className="px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400"
                      >
                        {code}
                      </span>
                    ))}
