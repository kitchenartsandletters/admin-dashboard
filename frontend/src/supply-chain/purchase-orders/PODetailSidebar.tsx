...
function InlineLineEntry({ poId, existingItemIds, onLineAdded }: {
  ...
  const handleAdd = async (v: VariantSearchResult) => {
    ...
    if (existingItemIds.has(v.inventory_item_id)) {
      setError(`"${v.title}" is already on this PO.`)
      setResults([]); setQuery('')
      return
    }
    ...
    try {
      ...
      setError(msg.includes('500') || msg.includes('fetch')
        ? 'Could not add this title — it may belong to a different supplier.'
        : msg)
    } finally {
      setAdding(null)
    }
    ...
    <div className="space-y-1.5" ref={ref}>
      <div className="flex gap-1.5">
        ...
        <input type="number" min={1} value={qty}
          ...
          className="w-12 px-1.5 py-1.5 border rounded text-xs text-center dark:bg-gray-900 dark:text-white dark:border-gray-700 focus:ring-1 focus:ring-blue-500 outline-none"
          title="Qty" />
      </div>
      {results.length > 0 && (
          ...
          {results.slice(0, 10).map(r => {
            ...
            return (
              <button key={r.inventory_item_id} type="button"
                ...
                className={`w-full text-left px-2.5 py-2 border-b dark:border-gray-800 last:border-0 text-xs
                  ${dup ? 'opacity-40 cursor-not-allowed' : 'hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50'}`}>
...
const PODetailSidebar: React.FC<Props> = ({ detail, onClose, onReceive, onRefresh, wide }) => {
  ...
  return createPortal(
    <>
      ...
      />
      <div
        className={`fixed top-0 right-0 h-full w-full ${sidebarWidth} bg-white dark:bg-gray-950 border-l border-gray-200 dark:border-gray-800 shadow-2xl z-50 flex flex-col transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header */}
